 ...
 */
const SellWindow = ({ uid, stockName = "NHPC", stockSymbol, stockPrice = 0, stockChange = 0, stockChangePercent = 0, onClose, onSwitchToBuy }) => {
    ...
    const handleSell = async () => {
        try {
            ...
            if (response.success) {
                ...
                showToast(`Sell Order Placed! ID: ${response.data.angelOrderId || response.data.orderId}`, "success");
                onClose();
            } else {
    ...
    return (
        <Draggable nodeRef={nodeRef} handle=".draggable-header">
            ...
            >
                {/* Header Section */}
                <div className="draggable-header cursor-move bg-[#f23645]/10 p-3 border-b border-[#f23645]/20 flex justify-between items-start">
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 text-sm mt-1">
                            <span className="font-semibold text-[#f23645]">{currentLivePrice.toFixed(2)}</span>
                            <span className="text-[#f23645] text-xs">{currentLiveChange.toFixed(2)} ({currentLiveChangePercent.toFixed(2)}%)</span>
                            <input type="radio" name={`exchange-${uid}`} id={`nse-${uid}`} defaultChecked className="accent-[#f23645]" />
                            <label htmlFor={`nse-${uid}`} className="text-xs text-[var(--text-muted)]">NSE</label>
                            <input type="radio" name={`exchange-${uid}`} id={`bse-${uid}`} className="accent-[#f23645]" />
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex bg-[var(--bg-secondary)] rounded p-0.5">
                            <button onClick={onSwitchToBuy} className="px-3 py-0.5 text-[var(--text-muted)] text-xs font-bold hover:text-[var(--text-primary)] transition-colors">B</button>
                            <button className="px-3 py-0.5 bg-[#f23645] text-white text-xs font-bold rounded shadow-sm">S</button>
                        </div>
                        <button onClick={onClose} className="p-1 hover:bg-[var(--bg-secondary)] rounded text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X size={16} /></button>
                    </div>
                </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex bg-[var(--bg-secondary)] rounded p-1 w-fit">
                                    <button
                                        ...
                                        className={`px-4 py-1.5 text-xs font-bold rounded transition-colors ${productType === "INT" ? "bg-[#f23645]/20 text-[#f23645] border border-[#f23645]/30" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
                                    >
                                        INT
                                    </button>
                                    <button
                                        onClick={() => setProductType("DEL")}
                                        className={`px-4 py-1.5 text-xs font-bold rounded transition-colors ${productType === "DEL" ? "bg-[#f23645]/20 text-[#f23645] border border-[#f23645]/30" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
                                    >
                                        DEL
                                    </button>
                                </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-24 bg-[var(--bg-secondary)] text-[var(--text-primary)] p-2 rounded border border-[var(--border-primary)] focus:border-[#f23645] focus:outline-none text-right"
                                    />
                                </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className={`w-28 bg-[var(--bg-secondary)] text-[var(--text-primary)] p-2 rounded border border-[var(--border-primary)] focus:border-[#f23645] focus:outline-none text-right ${isMarket ? "opacity-50 cursor-not-allowed" : ""}`}
                                    />
                                </div>
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <div
                                        ...
                                        className={`w-8 h-4 rounded-full p-0.5 cursor-pointer transition-colors ${isMarket ? "bg-[#f23645]" : "bg-[var(--bg-secondary)]"}`}
                                    >
                                    ...
                                    <span className={`text-[10px] ${isMarket ? "text-[#f23645] font-bold" : "text-[var(--text-muted)]"}`}>Market</span>
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-[var(--bg-main)] p-4 border-t border-[var(--border-primary)] flex justify-between items-center">
                    <div className="flex gap-6">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-[#f23645] font-semibold uppercase">Available Margin</span>
                            <span className="text-sm font-bold text-[var(--text-primary)]">₹ {userBalance.toFixed(2)}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[10px] text-[#f23645] font-semibold uppercase">Charges</span>
