...
const SearchBar = ({ onAddStock, onSelectStock, onBuy, onSell }) => {
    ...
    return (
        <>
            ...
            <div className="p-3 relative z-[100]" ref={containerRef}>
                <div className={`flex items-center bg-[#1e222d] rounded-md px-3 py-2 border transition-colors ${isFocused ? 'border-[#2962ff]' : 'border-[#2a2e39]'}`}>
                    ...
                    {isLoading ? (
                        ...
                        <button onClick={clearSearch} className="text-[#868993] hover:text-white transition-colors">
                            <X size={16} />
                        </button>
                    ) : !isFocused && (
            ...
            {isFocused && (
                <div className="absolute top-[60px] left-0 right-0 bottom-0 bg-[#0b0e14] z-[90] overflow-y-auto customscrollbar border-t border-[#2a2e39]">
                    ...
                    <div className="sticky top-0 bg-[#0b0e14] z-10 px-4 pt-2 border-b border-[#2a2e39] flex gap-6 text-sm font-medium text-[#868993] mb-2">
                        <span className="text-[#2962ff] border-b-2 border-[#2962ff] pb-2 cursor-pointer">All</span>
                        <span className="pb-2 cursor-pointer hover:text-[#d1d4dc]">Indices</span>
                        <span className="pb-2 cursor-pointer hover:text-[#d1d4dc]">Cash</span>
                        <span className="pb-2 cursor-pointer hover:text-[#d1d4dc]">F&O</span>
                        <span className="pb-2 cursor-pointer hover:text-[#d1d4dc]">MF</span>
                    </div>
                            ...
                            results.map((result) => (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-3">
                                        {/* EQ Badge / Avatar */}
                                        <div className="w-8 h-8 rounded-full border border-[#2a2e39] flex items-center justify-center text-[10px] text-[#2962ff] bg-[#2962ff]/10 font-bold">
                                            EQ
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[9px] bg-[#2a2e39] text-[#868993] px-1 py-0.5 rounded leading-none">
                                                    {result.exch_seg}
                                                </span>
                                            </div>
